...
class RTLearner():  		  	   		 	 	 			  		 			     			  	 
    ...
    def build_tree(self, data): # Function to build random decision tree.
        if data.shape[0] <= self.leaf_size: 
            return np.array((-1, stats.mode(data[:,-1])[0], np.nan, np.nan), dtype=float)   # Compute mode for leaf node value.
        elif np.unique(data[:,-1]).shape[0]==1:
            return np.array((-1, data[:,-1][0], np.nan, np.nan), dtype=float)
        else:
            ...
            if lefttree.ndim == 1:
                root = np.array((select_f, split_val, 1, 2), dtype=float)
            else:
                root = np.array((select_f, split_val, 1, lefttree.shape[0]+1), dtype=float)
